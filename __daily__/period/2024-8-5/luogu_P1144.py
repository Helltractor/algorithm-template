...
# https://github.com/liupengsay/PyIsTheBestLang/blob/main/src/graph/dijkstra/template.py
def get_cnt_of_shortest_path(dct, src: int, mod: int = -1):
...
def solve():
    ...
    for _ in range(m):
        ...
        g[a].append((b, 1))
        g[b].append((a, 1))
    cnt, dis = get_cnt_of_shortest_path(g, 1, 100003)
    for i in range(1, n + 1):
        print(cnt[i])


def luogu_P1144() -> None:
    for _ in range(1):
        solve()
